...
with zipfile.ZipFile(out, "w", zipfile.ZIP_DEFLATED) as zf:
    for path in list((ROOT / "checkpoints" / "baseline_sft").rglob("*")) + list((ROOT / "checkpoints" / "tier2").rglob("*")) + list((ROOT / "results").glob("*.json")):
        if path.is_file():
            zf.write(path, path.relative_to(ROOT))
print("RESULTS READY:", out, out.stat().st_size // 1_000_000, "MB", flush=True)
