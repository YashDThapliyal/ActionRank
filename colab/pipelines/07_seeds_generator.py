"""Detached on the Colab VM: two seed replicates of the final 3-epoch generator (configs/seeds/gen_s2.yaml, gen_s3.yaml).
...
for seed in SEEDS:
    cfg = f"configs/seeds/gen_s{seed}.yaml"
    env = {"ACTIONRANK_CONFIG": cfg}
    sh(f"grep -nE 'train_seed|sft_|results_dir' {cfg}", env)
    sh("python -u baseline_sft.py 2>&1 | grep --line-buffered -v 'HTTP Request'", env)
    package(f"gen-seed{seed}-train")
    sh("python -u eval.py --systems baseline_sft --offset 503 --all-remaining 2>&1 | grep --line-buffered -v 'HTTP Request'", env)
    package(f"gen-seed{seed}-eval")
log("ALL DONE")
