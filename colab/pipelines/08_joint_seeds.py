"""Detached on the Colab VM: Experiment A, Arm J: the span scorer trained with GenRec's joint ranking + LM objective, seeds 1 to 3 (configs/genrec/joint_s*.yaml).
...
def package(stage: str) -> None:
    ...
    with zipfile.ZipFile(out, "w", zipfile.ZIP_DEFLATED) as zf:
        for folder in ("checkpoints/genrec", "results/08-genrec-joint"):
...
for seed in SEEDS:
    cfg = f"configs/genrec/joint_s{seed}.yaml"
    env = {"ACTIONRANK_CONFIG": cfg}
    sh(f"grep -nE 'train_seed|lm_weight|lm_scope|pooling|^  head:|max_train_examples|^  epochs|batch_size|grad_accum|checkpoint_dir|results_dir' {cfg}", env)
    sh("python -u train_tier2.py 2>&1 | grep --line-buffered -v 'HTTP Request'", env)
    package(f"joint-seed{seed}-train")
    sh("python -u eval.py --systems tier2 --offset 503 --all-remaining 2>&1 | grep --line-buffered -v 'HTTP Request'", env)
    package(f"joint-seed{seed}-eval")
log("ALL DONE")
