...
- `ACTIONRANK_DEVICE` and `ACTIONRANK_CONFIG` override the device or the whole config (e.g. `config_last.yaml` for last-token pooling, `config_last_span.yaml` for the fine-tuned span scorer).
- Fine-tuning runs (`train_tier2.py`, `baseline_sft.py`) work locally but are meant for a CUDA GPU. `colab/` has a detached pipeline for the Colab CLI: `colab exec` drops the connection after ~60 s of silence, so the pipeline logs to a file and is polled with `colab/tail_remote.py`.
